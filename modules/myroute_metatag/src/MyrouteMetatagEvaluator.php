 ...
 *
 * @package Drupal\myroute_metatag
 */
class MyrouteMetatagEvaluator {
  ...
  /**
   * @var array $evaluations_by_route_name ;
   */
  ...
  public function evaluateByRouteName($route_name) {
    if (!isset($this->evaluations_by_route_name[$route_name])) {
      ...
      if (!empty($myroute_metatags)) {
        foreach ($myroute_metatags as $myroute_metatag) {
          /** @var \Drupal\myroute_metatag\Entity\MyrouteMetatag $myroute_metatag */
          if ($this->evaluate($myroute_metatag)) {
            $this->evaluations_by_route_name[$route_name] = $myroute_metatag->id();
   ...
   * @return boolean
   */
  public function evaluate(MyrouteMetatag $myroute_metatag) {
    $id = $myroute_metatag->id();
    if (!isset($this->evaluations[$id])) {
      /** @var ConditionPluginCollection $conditions */
      $conditions = $myroute_metatag->getConditions();
      if (empty($conditions)) {
        return TRUE;
      }
      $logic = $myroute_metatag->getLogic();
